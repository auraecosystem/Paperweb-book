...
Click on your server in the list to show the default web site service in the list
Choose New->Virtual Directory from the Action menu
Click Next when the Virtual Directory Creation Wizard window appears
Enter the name htmldoc in the Alias field and click Next
Enter the HTMLDOC program folder in the Directory field and click Next
Check the Execute (such as ISAPI applications or CGI) box and click Next
...
Click Add... and choose the htmldoc.exe file from the program folder, typically C:\Program Files\msweet.org\HTMLDOC
...
Click Add..., enter the username for the web server, typically "SERVER\IUSR_SERVER" where "SERVER" is the name you gave your server, and click OK
...
HTMLDOC encountered a networking error when retrieving a file via a URL.
HTMLDOC was unable to read a file.
HTMLDOC was unable to write a file.
A HTML error was found in a source file.
A table, image, or text fragment was too large to fit in the space provided.
